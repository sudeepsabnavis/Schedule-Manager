			...
			if(preg_match('#^(([a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+\.?)*[a-z0-9!\#$%&\\\'*+/=?^_`{|}~-]+)@(([a-z0-9-_]+\.?)*[a-z0-9-_]+)\.[a-z]{2,}$#i',$_POST['email']))
			{
				...
				if($dn==0)
				{
					...
					if(mysql_query('insert into users(id, username, password, email, avatar, signup_date) values ('.$id.', "'.$username.'", "'.$password.'", "'.$email.'", "'.$avatar.'", "'.time().'")'))
					{
						...
						$dn2 = mysql_num_rows(mysql_query('select id from users'));
$id = $dn2;
$req1 = mysql_query(' select username from users where id='.$id.' ');
$name = mysql_fetch_array($req1);
$name1=$name[0];

mysql_query("CREATE TABLE `".$name1."` ( id VARCHAR(30) ,timeslot1 VARCHAR(30) ,event1 VARCHAR(30) , timeslot2 VARCHAR(30) , event2 VARCHAR(30) , timeslot3 VARCHAR(30) NOT NULL, event3 VARCHAR(30) , timeslot4 VARCHAR(30) , event4 VARCHAR(30) , timeslot5 VARCHAR(30) , event5 VARCHAR(30) , timeslot6 VARCHAR(30)  , event6 VARCHAR(30) , timeslot7 VARCHAR(30) , event7 VARCHAR(30) , timeslot8 VARCHAR(30) ,event8 VARCHAR(30))");
...
?>
		<div class="foot"><a href= home.php>Go Home</a> </div>
	</body>
</html>